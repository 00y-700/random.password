...
generate.addEventListener("click", () => {
  ...
  const iLC = lc.checked;
  const iUC = uc.checked;
  const iNM = nm.checked;
  const iSC = sc.checked;
  
  out.innerText = generatePassword(iLC, iUC, iNM, iSC, len);
};

function generatePassword(LC, UC, NM, SC, passLength) {
  let generatedPassword = '';

  const typesCount = LC + UC + NM + SC;
